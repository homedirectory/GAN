...
LABELS_NAMES = ["T-shirt", "Trousers", "Pullover", "Dress", "Coat", "Sandal", "Shirt", "Sneaker", "Bag", "Ankle boot"]
...
# split the whole dataset into subdatasets for each label
...
def train(epochs, load_models=False, save_images=False, save_models=False):
    ...
    if load_models:
        try:
            models = [(tf.keras.models.load_model(f"models/latest/{l}/critic"), tf.keras.models.load_model(f"models/latest/{l}/generator")) for l in LABELS_NAMES]
        except:
            raise SystemExit("####### Failed to load models ########")
            
    else:
        models = [WGAN_GP().models for _ in LABELS_NAMES]
    ...
    img_save_paths = ["" for _ in range(len(LABELS_NAMES))]
    if save_images:
        img_save_dir = f"generated/{basename}"
        os.mkdir(img_save_dir)
        img_save_paths = [f"{img_save_dir}/{l}" for l in LABELS_NAMES]
        for p in img_save_paths:
            os.mkdir(p)

    save_model_every = max(1, int(epochs / 5))
    # create a dir to save models
    models_save_paths = ["" for _ in range(len(LABELS_NAMES))]
    if save_models:
        models_save_dir = f"models/{basename}"
        os.mkdir(models_save_dir)
        models_save_paths = [f"{models_save_dir}/{l}" for l in LABELS_NAMES]
        for p in models_save_paths:
            os.mkdir(p)

    for m, dataset, isp, msp, lbl in zip(models, TRAIN_DATASETS, img_save_paths, models_save_paths, LABELS_NAMES): 
        print(f"----------- TRAINING TO GENERATE **{lbl}** -----------")
